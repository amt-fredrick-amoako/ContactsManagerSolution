...
namespace Services
{
    public class PersonsGetterServiceWithFewExcelFields : IPersonsGetService
    {
        private readonly PersonsGetService _personsGetService;

        public PersonsGetterServiceWithFewExcelFields(PersonsGetService personsGetService)
        {
        ...
        {
            return await _personsGetService.GetAllPersons();
        }

        public async Task<List<PersonResponse>> GetFilteredPersons(string searchBy, string searchString)
        {
            return await _personsGetService.GetFilteredPersons(searchBy, searchString);
        }

        public async Task<PersonResponse?> GetPersonByPersonId(Guid? id)
        {
            return await _personsGetService.GetPersonByPersonId(id);
        }

        public async Task<MemoryStream> GetPersonsCSV()
        {
            return await _personsGetService.GetPersonsCSV();
        }
